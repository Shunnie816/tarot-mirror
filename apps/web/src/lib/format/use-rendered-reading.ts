...
/**
 * ここで待つのは「もう出来上がっているか」だけ。
 *
 * 整形そのものは3枚で5〜7秒、8枚だと18秒かかる（実測）。それを読み物の手前で
 * 待つと、本文を伏せたまま十数秒眺めさせることになり、文章が入れ替わるより
 * 悪い。だから作るのはカードを置いている最中に済ませておき（`PrefetchReading`）、
 * ここではサーバー側のキャッシュに載っているかどうかだけを見る。
 *
 * 当たれば往復 0.3 秒ほど。当たらなければ待たずにテンプレートで出す。取りこぼした
 * 整形もサーバー側では完了してキャッシュに載るので、次に開けば出てくる。
 */
