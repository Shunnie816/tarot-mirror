...
**既定はオフ。** `/settings` で切り替えると Cookie（`tm.llm`）に残る。Cookie にしてある
のはサーバーが読めるからで、この設定は最初の描画そのものを変える（整えるなら本文を
伏せて待ち、整えないならすぐ出す）。サーバーが知らないと、既定の無料経路にまで
「待っている」画面を挟むか、整える経路で本文が一瞬出てから消えるかのどちらかになる。
...
Phase 1–9 完了（262 tests + エミュレータ 37 tests / typecheck clean）。
